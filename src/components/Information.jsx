...
const Information = (props) => {
    ...
    const onMessageReceived = async (e) => {
        switch (e.data.status) {
            ...
            case 'update':
                ...
                break;
            case 'complete':
              setTranslation(e.data.output); // ← This should be an array of full translated lines
  ...
  function handleDownload() {
    ...
    if (tab === 'translation' && Array.isArray(translation)) {
      // ✅ Use the full completed translation from the backend
      textToSave = translation.join('\n');
    } else {
      // Use transcription or fallback
      textToSave = Array.isArray(textElement) ? textElement.join('\n') : textElement;
    }
  
    const element = document.createElement("a");
    const file = new Blob([textToSave], { type: 'text/plain' });
    element.href = URL.createObjectURL(file);
    element.download = `Freescribe_${new Date().toISOString()}.txt`;
